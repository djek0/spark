...
private[spark] class TaskSchedulerImpl(
  ...
  // Task replication: Track task index to host mapping for replica separation
  ...
  // Task replication: Check if replica task would be assigned to same host as its partner
          ...
          try {
            ...
            for (task <- taskDescOption) {
              val (locality, resources) = if (task != null) {
                tasks(i) += task
                // Task replication: Track task-to-host mapping and check for illegal assignments
                val taskIndex = task.index.toLong
                checkForIllegalTaskAssignment(taskIndex, host)
