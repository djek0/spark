...
private[spark] class TaskSetManager(
    ...
    : (Option[TaskDescription], Boolean, Int) =
  {
    ...
    if (!isZombie && !offerExcluded) {
      ...
      // Task replication: pass stageId for replica host separation
      var dequeuedTaskIndex: Option[Int] = None
      val taskDescription =
        dequeueTask(execId, host, allowedLocality, taskSet.stageId)
  ...
  def handleSuccessfulTask(tid: Long, result: DirectTaskResult[_]): Unit = {
    ...
    // Note: This only kills attempts of the same task INDEX - replica partners have different indices
    // and are handled by DAGScheduler batching, so they won't be killed here.
