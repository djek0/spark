...
abstract class ShuffleSuite extends SparkFunSuite with Matchers with LocalRootDirsTest {
  ...
  test("multiple simultaneous attempts for one task (SPARK-8029)") {
    ...
    val context1 =
      new TaskContextImpl(0, 0, 0, 0L, 0, 0, taskMemoryManager, new Properties, metricsSystem)
    ...
    val context2 =
      new TaskContextImpl(0, 0, 0, 1L, 0, 0, taskMemoryManager, new Properties, metricsSystem)
    ...
    val taskContext = new TaskContextImpl(
      1, 0, 0, 2L, 0, 0, taskMemoryManager, new Properties, metricsSystem)
