...
run_test() {
    ...
    if [ -z "$env_vars" ]; then
        timeout 120s ./bin/run-example ComplexSparkTest 2>&1 | grep -E "CONSENSUS|VERIFICATION|BYZANTINE|Final Output|Group.*sum|Verdict|REPLICA.*CORRECT|NEITHER_MATCH" | tail -30 | tee -a "$RESULTS_FILE"
        exit_code=${PIPESTATUS[0]}
    else
        timeout 120s env $env_vars ./bin/run-example ComplexSparkTest 2>&1 | grep -E "CONSENSUS|VERIFICATION|BYZANTINE|Final Output|Group.*sum|Verdict|REPLICA.*CORRECT|NEITHER_MATCH" | tail -30 | tee -a "$RESULTS_FILE"
        exit_code=${PIPESTATUS[0]}
    fi
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        echo "⏱️  TIMEOUT (120s)" | tee -a "$RESULTS_FILE"
        duration="TIMEOUT"
        status="TIMEOUT"
    else
        echo "❌ FAILED - Exit code: $exit_code - Duration: ${duration}s" | tee -a "$RESULTS_FILE"
        status="FAIL"
    fi
...
result=$(run_test "2" "Driver Full Task" "HONEST=False" "BYZANTINE_PROBABILITY=3" "EXEC_VERIFICATION=false" "MERKLE_VERIFICATION=false")
...
result=$(run_test "3" "Driver Merkle (Trees on Driver)" "HONEST=False" "BYZANTINE_PROBABILITY=3" "EXEC_VERIFICATION=false" "MERKLE_VERIFICATION=true" "MERKLE_BUILD_ON_DRIVER=true")
...
result=$(run_test "4" "Driver Merkle (Trees on Executors)" "HONEST=False" "BYZANTINE_PROBABILITY=3" "EXEC_VERIFICATION=false" "MERKLE_VERIFICATION=true" "MERKLE_BUILD_ON_DRIVER=false")
...
result=$(run_test "5" "3rd Executor Full Task" "HONEST=False" "BYZANTINE_PROBABILITY=3" "EXEC_VERIFICATION=true" "MERKLE_VERIFICATION=false")
...
result=$(run_test "6" "3rd Executor Merkle (Trees on Driver)" "HONEST=False" "BYZANTINE_PROBABILITY=3" "EXEC_VERIFICATION=true" "MERKLE_VERIFICATION=true" "MERKLE_BUILD_ON_DRIVER=true")
...
result=$(run_test "7" "3rd Executor Merkle (Trees on Executors)" "HONEST=False" "BYZANTINE_PROBABILITY=3" "EXEC_VERIFICATION=true" "MERKLE_VERIFICATION=true" "MERKLE_BUILD_ON_DRIVER=false")
